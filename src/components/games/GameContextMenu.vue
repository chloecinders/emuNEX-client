...
const open = (event: MouseEvent, id: string) => {
    ...
    // Use nextTick equivalent or setTimeout to wait for DOM
    setTimeout(() => {
...
<template>
    ...
    >
        ...
        <button v-if="isInstalled" class="c-context-menu__item c-context-menu__item--danger" @click="deleteGame">Delete Game</button>
    ...
    <Modal :show="showDeleteModal" title="Delete Game Data" @close="showDeleteModal = false">
        <div style="display: flex; flex-direction: column; gap: var(--spacing-lg); padding: var(--spacing-sm) 0;">
            <Text variant="muted">Are you sure you want to delete this game? This will remove the game files from your system.</Text>
            
            <div style="padding: var(--spacing-md); background: var(--color-surface-variant); border-radius: var(--radius-md); border: 1px solid var(--color-border);">
            ...
            <div style="display: flex; gap: var(--spacing-md); justify-content: flex-end; margin-top: var(--spacing-sm);">
                ...
                <Button @click="confirmDeleteGame" style="background: var(--color-red, #ef4444); color: white;">Delete</Button>
            </div>
        </div>
    </Modal>
</template>
